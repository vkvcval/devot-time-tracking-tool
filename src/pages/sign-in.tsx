  ...
  const handleLogin = async (e: MouseEvent<HTMLButtonElement>) => {
    ...
    if (error) {
      if (error.code === 'auth/user-not-found' || error.code === 'auth/wrong-password') {
        ...
        showToastMessage({ severity: 'error', summary: 'Invalid email.' });
      } else {
        showToastMessage({ severity: 'error', summary: 'Unable to login.' });
      }
    }
  };
        ...
        <div>
          <div className={styles.loginFormWrapper}>
            ...
            <form className={styles.form}>
              ...
              />
              <Button className={styles.loginBtn} label='Login' onClick={handleLogin} />
            </form>
          </div>
          <RegisterUser className={styles.registerUser} />
        </div>
      </div>
    </Layout>
  );
